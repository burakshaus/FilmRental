    ...
    public class SeedController : ControllerBase
    {
        ...
        public async Task<IActionResult> GenerateEmbeddings()
        {
            ...
            if (string.IsNullOrEmpty(apiKey))
                return BadRequest("OpenRouter API Key eksik.");

            // Bütün filmleri (eski Gemini vektörü olanları da) baştan çek ve Qwen/Nomic vektörüyle değiştir!
            ...
            if (!movies.Any())
                return Ok(new { message = "Seeded film bulunamadı." });
            ...
            foreach (var movie in movies)
            {
                try
                {
                    ...
                    var embedding = await GetEmbeddingAsync(apiKey, text);
                    if (embedding != null)
                    {
